...
// File contracts/token/Test1/Cheems1.sol


// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.6;
contract Cheems is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for *;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    constructor(string memory _name, string memory _symbol, uint256 _maxSupply, uint256 _claimAmount, uint256 _maxClaimCount) ERC20(_name, _symbol) {
        _mint(_msgSender(), _maxSupply.div(2));
        _mint(address(this), _maxSupply.div(2));
        rewardManager = owner();
        claimAmount = _claimAmount;
        maxClaimCount = _maxClaimCount;
    }

    function claim(bytes memory signature, uint256 timestamp) public nonReentrant {
        console.log("timestamp", timestamp);
        console.log("block.timestamp", block.timestamp);
        console.log("rewardManager", rewardManager);

        // only one
        require(!claimedAddresses[_msgSender()], "can not claim twice!");
        // get expect message hash
        require(verify(keccak256(abi.encodePacked(_msgSender(), timestamp)), signature, rewardManager), "invalid signature");
        // check max user claim count
        require(claimCount.current() < maxClaimCount, "max claim count exceeded");
    ...
    function claimFull() public view returns (bool) {
        return claimCount.current() >= maxClaimCount;
    }

    function claimed() public view returns (bool) {
        return claimedAddresses[_msgSender()];
    }
    ...
    function claimStart() external onlyOwner {
        claimEnable = true;
    }

    function claimDisable() external onlyOwner {
        claimEnable = false;
    }

    function claimEnd() external onlyOwner {
    ...
    function setMaxClaimCount(uint256 _amount) external onlyOwner {
        maxClaimCount = _amount;
    }
